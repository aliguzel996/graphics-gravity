...
<div class="app">
  <header>
    <div class="brand">GRAPHICS GRAVITY <small>YCSWU / 0.1</small></div>
  ...
  <main>
    <aside>
      <section class="section">
        <div class="section-title"><span>01 / inputs</span><span id="itemCount">0 items</span></div>
        ...
        <details class="appearance-panel">
          ...
          <label class="check"><input id="previewGrid" type="checkbox" checked> show preview grid</label>
      ...
      <section class="section">
        <div class="section-title"><span>02 / piece size</span><span>live</span></div>
      ...
      <section class="section">
        <div class="section-title"><span>03 / container</span><span id="containerLabel">circle</span></div>
      ...
      <section class="section">
        <div class="section-title"><span>04 / physics</span><span>live</span></div>
      ...
      <section class="section"><div class="quiet">INPUTS DROP LEFT TO RIGHT. DRAG INPUT CARDS TO REORDER THE QUEUE. CLICK A CARD TO MAKE IT NEXT.</div></section>
    </aside>
    <div class="stage-wrap"><div class="stage frame-free" id="stage"><div class="stage-tools stage-tools-left"><button id="recBtn">REC</button><span id="recTime">00:00</span></div><div class="stage-tools stage-tools-center"><button class="active" data-frame="free">FREE</button><button data-frame="square">1:1</button><button data-frame="vertical">9:16</button></div><div class="stage-tools stage-tools-right"><button id="svgBtn">EXPORT SVG</button><button id="jpgBtn">EXPORT JPG</button><button id="pngBtn">EXPORT PNG</button></div><div class="frame-resolution" id="frameResolution"></div><div class="stage-tip" id="stageTip">click or hold to drop the input queue left to right</div><div class="draw-note">click points around the desired boundary, then close shape</div><div class="status"><span id="bodyStatus">0 bodies</span><span id="fpsStatus">60 fps</span></div><div class="zoom-level" id="zoomLevel">ZOOM 100%</div></div></div>
  </main>
...
function drawLibrary() {
  ...
  library.forEach((item,index) => {
    ...
    tile.draggable = true; tile.dataset.index = index; tile.dataset.order = index + 1;tile.dataset.itemId=item.id;if(vector){const stroke=vectorStrokeAppearance();tile.style.setProperty('--input-color',itemEffectiveColor(item));tile.style.setProperty('--input-opacity',itemEffectiveOpacity(item)/100);tile.style.setProperty('--input-paint',itemEffectivePaint(item));tile.style.setProperty('--input-stroke-color',stroke.enabled?stroke.paint:'transparent');tile.style.setProperty('--input-stroke-width',stroke.enabled?`${Math.min(3,stroke.thickness)}px`:'0');}
...
function spawn(x,y) {
  ...
  if(item.kind==='text') {
    const h=size*1.25,w=item.value.length===1?h:Math.max(size,item.value.length*size*.62),radiusValue=+document.querySelector('#textRadius').value;
    body=createTextBody(x,y,w,h,item.value,radiusValue,opts);
    body.plugin={drawText:item.value,fontSize:size*.75,textWidth:w,textHeight:h,textPhysicsCircle:item.value.length===1&&radiusValue>=100,textPhysicsRadius:radiusValue};
...
document.querySelector('#addText').onclick=()=>{const v=document.querySelector('#textInput').value.trim();if(v)addLibrary({kind:'text',value:v,preview:v,color:colorValue('#textColor','#111111')});};
...
function savedBody(body){
  ...
  if(body.plugin?.drawText)return{...base,kind:'text',text:body.plugin.drawText,fontSize:body.plugin.fontSize,textWidth:body.plugin.textWidth,textHeight:body.plugin.textHeight,textPhysicsRadius:body.plugin.textPhysicsRadius};
...
function projectSnapshot(){return{format:PROJECT_FORMAT,version:PROJECT_VERSION,savedAt:new Date().toISOString(),canvas:{width:W,height:H},frameMode,paused,textInput:document.querySelector('#textInput').value,settings:savedSettings(),container:{type:containerType,angle:containerAngle,customPoints:customPoints.map(point=>[...point]),drawing},library:savedLibrary(),queueIndex,selectedInputId,itemId,bodies:engine.world.bodies.filter(body=>body.label==='piece').map(savedBody)};}
...
function restoreControlOutputs(){
  ...
  text('globalSizeOut',`${value('globalPieceSize')}%`);text('randomMinOut',`${value('randomMin')}%`);text('randomMaxOut',`${value('randomMax')}%`);text('sizeOut',`${value('containerSize')}%`);text('sidesOut',String(value('containerSides')));text('rectLongOut',`${value('rectLongSide')}%`);text('rectShortOut',`${value('rectShortSide')}%`);text('thicknessOut',`${value('outlineThickness')} px`);text('vectorStrokeOut',`${value('vectorStrokeThickness')} px`);text('textBorderOut',`${value('textBorderThickness')} px`);text('textRadiusOut',`${value('textRadius')}%`);text('holdDropOut',`${value('holdDropRate')} / sec`);text('rotationOut',`${value('rotationSpeed')} deg/s`);text('gravityOut',(value('gravity')/100).toFixed(2));text('bounceOut',(value('bounce')/100).toFixed(2));text('frictionOut',(value('friction')/100).toFixed(2));text('attractOut',(value('attraction')/100).toFixed(2));
  lastGlobalSize=value('globalPieceSize')/100;lastContainerSize=value('containerSize');engine.gravity.scale=.001*value('gravity')/100;render.options.background='transparent';syncHexColorFields();syncVectorStrokeControls();
}
...
function restoredBody(snapshot,sx,sy,uniform){
  ...
  if(snapshot.kind==='text'){const w=snapshot.textWidth*uniform,h=snapshot.textHeight*uniform;body=createTextBody(position.x,position.y,w,h,snapshot.text,snapshot.textPhysicsRadius,opts);body.plugin={drawText:snapshot.text,fontSize:snapshot.fontSize*uniform,textWidth:w,textHeight:h,textPhysicsCircle:snapshot.text.length===1&&snapshot.textPhysicsRadius>=100,textPhysicsRadius:snapshot.textPhysicsRadius};}
...
async function restoreProject(project){
  ...
  const items=await hydratedLibrary(project.library,project.settings||{});stopDropping();if(mediaRecorder?.state==='recording')stopRecording();engine.world.bodies.filter(body=>body.label==='piece').forEach(body=>Composite.remove(engine.world,body));
...
document.querySelector('#vectorColor').oninput=e=>{setColorFieldValue(document.querySelector('#textColor'),e.target.value);library.filter(itemIsVector).forEach(item=>setItemVectorColor(item,e.target.value));};
...
bindOpacity('#vectorColor',value=>{setOpacityFieldValue(document.querySelector('#textColor'),value);library.filter(itemIsVector).forEach(item=>setItemVectorOpacity(item,value));});
...
function svgPiece(body){
  if(body.plugin?.drawText){const angle=svgNumber(body.angle*180/Math.PI),text=svgEscape(body.plugin.drawText),fontSize=svgNumber(body.plugin.fontSize),item=library.find(entry=>entry.id===body.plugin.inputId),fill=svgFillAttributes(itemEffectiveColor(item),itemEffectiveOpacity(item)),stroke=svgVectorStrokeAttributes();return`<g transform="translate(${svgNumber(body.position.x)} ${svgNumber(body.position.y)}) rotate(${angle})">${svgTextBackground(body)}<text x="0" y="0" text-anchor="middle" dominant-baseline="central" font-family="Space Mono, monospace" font-size="${fontSize}" font-weight="400" ${fill}${stroke}>${text}</text></g>`;}
...
function exportSvg(){
  ...
  const defs=hasContainer?`<defs><clipPath id="containerClip"><polygon points="${points}"/></clipPath></defs>`:'',containerFill=hasContainer?`<polygon points="${points}" ${container}/>`:'';
