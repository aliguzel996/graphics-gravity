...
<div class="app">
  <header>
    <div class="brand">GRAPHICS GRAVITY <small>YCSWU / 0.2.2</small></div>
...
async function downloadBlob(blob,name){
  if(window.graphicsGravityFile?.saveExport)return window.graphicsGravityFile.saveExport(await blob.arrayBuffer(),name,blob.type);
  const a=document.createElement('a'),url=URL.createObjectURL(blob);a.download=name;a.href=url;document.body.append(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),60000);return{canceled:false,name,size:blob.size};
}
...
function canvasBlob(canvas,type='image/png',quality){return new Promise((resolve,reject)=>canvas.toBlob(blob=>blob?resolve(blob):reject(new Error(`${type} export is unavailable`)),type,quality));}
async function exportPng(){return downloadBlob(await canvasBlob(renderSceneExportCanvas(),'image/png'),'graphics-gravity.png');}
async function exportJpg(){const scene=renderSceneExportCanvas(),output=document.createElement('canvas');output.width=W;output.height=H;const context=output.getContext('2d');context.fillStyle='#ffffff';context.fillRect(0,0,W,H);context.drawImage(scene,0,0);return downloadBlob(await canvasBlob(output,'image/jpeg',.94),'graphics-gravity.jpg');}
async function runExportButton(selector,task){const button=document.querySelector(selector),original=button.textContent;button.disabled=true;button.textContent='SAVING...';try{const result=await task();if(!result?.canceled){button.textContent='SAVED';setTimeout(()=>{button.textContent=original;button.disabled=false;},900);return;}button.textContent=original;button.disabled=false;}catch(error){console.error(`${original} failed`,error);button.textContent='ERROR';setTimeout(()=>{button.textContent=original;button.disabled=false;},1400);}}
document.querySelector('#pngBtn').onclick=()=>runExportButton('#pngBtn',exportPng);
document.querySelector('#jpgBtn').onclick=()=>runExportButton('#jpgBtn',exportJpg);
...
async function exportSvg(){
  ...
  try{
    ...
    await downloadBlob(new Blob([svg],{type:'image/svg+xml;charset=utf-8'}),'graphics-gravity.svg');
...
document.querySelector('#saveWebm').onclick=async()=>{if(!pendingRecordingBlob)return;const result=await downloadBlob(pendingRecordingBlob,`graphics-gravity-${Date.now()}.webm`);if(result?.canceled)return;pendingRecordingBlob=null;recordDialog.close();};
document.querySelector('#savePngSequence').onclick=async()=>{if(!pendingRecordingBlob)return;const blob=pendingRecordingBlob;recordDialog.classList.add('busy');try{const zip=await pngSequenceFromWebm(blob),result=await downloadBlob(zip,`graphics-gravity-png-sequence-${Date.now()}.zip`);if(result?.canceled){recordDialog.classList.remove('busy');recordDialogNote.textContent='Save canceled.';return;}pendingRecordingBlob=null;recordDialog.close();}catch(error){recordDialog.classList.remove('busy');recordDialogNote.textContent=`PNG sequence failed: ${error.message}`;}};
